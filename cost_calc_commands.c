...
/*   Updated: 2023/11/15 14:36:10 by sbelomet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_calculate_cost(t_stack **a, t_stack **b, t_stack *current)
{
	...
	if (atarget > alen / 2 && btarget > blen / 2)
		return (ft_calcrrarrb(a, b, current, ft_find_node_address(b, 'b')) + 1);
	else if (atarget > alen / 2)
		return (ft_calcrrarb(a, b, current, ft_find_node_address(b, 'b')) + 1);
	else if (btarget > blen / 2)
		return (ft_calcrarrb(a, b, current, ft_find_node_address(b, 'b')) + 1);
	return (ft_calcrarb(a, b, current, ft_find_node_address(b, 'b')) + 1);
}
